		...
		public static function add_admin_pages() {
			if (!defined('UPDRAFTPLUS_DISABLE_TOP_LEVEL_MENU_ENTRY') || !UPDRAFTPLUS_DISABLE_TOP_LEVEL_MENU_ENTRY) {
				if (is_super_admin() || apply_filters('updraft_user_can_manage', false, true)) {
					...
					if (!defined('WP_ALLOW_MULTISITE') || !WP_ALLOW_MULTISITE) unset($admin_page_hooks['settings.php?page=updraftplus']);
	...
	class UpdraftPlusAddOn_MultiSite {
		...
		// "Others", because on pre-WP-3.5-style networks, uploads are in wp-content/blogs.dir
			...
			while (!$page || (!empty($sites) && 100 == count($sites))) {
				...
				$sites = (function_exists('wp_get_sites') && version_compare($wp_version, '4.6.0', '<')) ? wp_get_sites($params) : $this->wp_get_sites($params);// phpcs:ignore WordPress.WP.DeprecatedFunctions.wp_get_sitesFound -- This function was only intended for backward compatibility with versions below 4.6.
