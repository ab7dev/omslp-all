...
class UpdraftPlus_Addons_RemoteStorage_sftp extends UpdraftPlus_RemoteStorage_Addons_Base_v2 {
	...
	private function do_connect_and_chdir() {
		...
		if (!empty($fingerprint)) {
			...
			if (!$match_fingerprint && isset($updraftplus->phpseclib2_supported_hostkey_algorithms)) {
				// phpseclib 3 prioritises and forces new host key algorithms like Ed25519 to be used when logging in to a server (if supported by the server)
				// post phpseclib upgrade from version 2 to 3, there could be an existing configuration that uses an older host key algorithm
				// if an existing fingerprint was made using the older algorithm whereas a connection made to a server uses a new algorithm then the fingerprint will fail to be validated
				// so we get the phpseclib2 supported hostkey algorithms and let these algorithms as the preferred ones
				$sftp = $this->connect($host, $port, $fingerprint, $user, $pass, $key, $scp, false, $updraftplus->phpseclib2_supported_hostkey_algorithms);
				if (is_wp_error($sftp)) return $sftp;
				if (!$this->validate_fingerprint($fingerprint)) return new WP_Error('invalid_fingerprint', __("Fingerprints don't match.", 'updraftplus'));
			}
		}
	 ...
	 * @param array   $preferred_hostkey_algorithms - List of preferred host key algorithms
	 *
	 * @return WP_Error|Net_SSH2|Net_SCP
	 */
	private function connect($host, $port = 22, $fingerprint = '', $user = '', $pass = '', $key = '', $scp = false, $debug = false, $preferred_hostkey_algorithms = array()) {// phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- Unused parameters are for future use.
		...
		if (!empty($preferred_hostkey_algorithms)) $this->ssh->setPreferredAlgorithms(array('hostkey' => $preferred_hostkey_algorithms));

		$this->server_public_hostkey = $this->ssh->getServerPublicHostKey();
		if ($this->server_public_hostkey) $this->server_public_hostkey = $this->maybe_convert_key_to_phpseclib_format($this->server_public_hostkey);

		if (!empty($key)) {
			$failed_to_load_private_key = false;
			if ($this->is_elliptic_curve_host_key_algorithm($this->server_public_hostkey) && class_exists('phpseclib3_Crypt_PublicKeyLoader')) {
				// handle server hostkey elliptic curve algorithm like Ed25519, ECDH, and ECDSA but we stick with the old phpseclib2 for RSA
				try {
					$key = $pass = call_user_func_array(array('phpseclib3_Crypt_PublicKeyLoader', 'load'), array($key)); // yes it's public key loader but it can also be used to load private key (@see https://github.com/phpseclib/phpseclib/blob/109ec382a957fd17251782c70cc1eb053c2877cc/phpseclib/Crypt/PublicKeyLoader.php#L30)
				} catch (Exception $e) {
					$failed_to_load_private_key = true;
				}
			} else {
			...
			if ($failed_to_load_private_key) {
				if (preg_match('/Encryption: (.+)/i', $key, $matches)) {
					$encryption = trim($matches[1]);
					if ('none' !== $encryption) return new WP_Error('no_key_passphrase', __("The key provided is encrypted.", 'updraftplus').' '.__("You need to provide the unencrypted key (see: https://teamupdraft.com/documentation/updraftplus/topics/cloud-storage/ftp-fstp/faqs/why-must-i-use-a-non-encrypted-sftp-key/).", 'updraftplus'));
				}
	...
	public function get_configuration_template() {
		...
		?>
			...
			<tr class="{{get_template_css_classes true}}">
				...
				<td>
					<input type="text" class="updraft_sftp_host_settings updraft_input--wide udc-wd-600" data-updraft_settings_test="host" id="{{get_template_input_attribute_value "id" "host"}}" name="{{get_template_input_attribute_value "name" "host"}}" value="{{host}}" />
					<br>
					<em class="updraft_sftp_host_error" style="display: none;">{{hostname_error_label}}</em>
				</td>
			</tr>
	...
	public function credentials_test($posted_settings) {
		...
		if ($putfile) {
			...
			$phpseclib3_class_shortname = $this->is_elliptic_curve_host_key_algorithm($this->server_public_hostkey) ? substr(get_parent_class($this->server_public_hostkey), strrpos(get_parent_class($this->server_public_hostkey), '\\') + 1) : 'RSA';
			/* translators: 1: Shortname for the host key algorithm in use, 2: Cryptographic hash function, 3: Hashed fingerprint */
			$credentials_test_success_msg = __('The server\'s %1$s key %2$s fingerprint: %3$s.', 'updraftplus');
			printf(' '.esc_html($credentials_test_success_msg).' ', $phpseclib3_class_shortname, 'MD5', esc_html($valid_fingerprints['md5']));
			printf(esc_html($credentials_test_success_msg), $phpseclib3_class_shortname, 'SHA256', esc_html($valid_fingerprints['sha256']));
		} else {
			if (empty($scp)) {
	...
	private function get_fingerprints($ssh = '') {
		...
		if (null === $this->server_public_hostkey) $this->server_public_hostkey = $ssh->getServerPublicHostKey();

		if (is_string($this->server_public_hostkey)) $this->server_public_hostkey = $this->maybe_convert_key_to_phpseclib_format($this->server_public_hostkey);
		
		if ($this->is_elliptic_curve_host_key_algorithm($this->server_public_hostkey)) {
			// handle server hostkey elliptic curve algorithm like Ed25519, ECDH, and ECDSA but we stick with the old phpseclib2 for RSA
			return array(
				'md5' => $this->server_public_hostkey->getFingerprint('md5'),
				'sha256' => $this->server_public_hostkey->getFingerprint('sha256'),
			);
		} else {
			$host_rsa = new phpseclib_Crypt_RSA();
			$host_rsa->loadKey($this->server_public_hostkey);
			return array(
	...
	/**
	 * Try to convert public or private key to phpseclib3 format
	 *
	 * @param string $key Public/private key
	 * @return object|string phpseclib3 object in which the key was converted to, original key otherwise
	 */
	private function maybe_convert_key_to_phpseclib_format($key) {
		if (!class_exists('phpseclib3_Crypt_PublicKeyLoader')) return $key;
		...
		if (isset($parts[1])) {
			try {
				$key = call_user_func_array(array('phpseclib3_Crypt_PublicKeyLoader', 'load'), array($parts[1]));
				$loaded = true;
			} catch (Exception $e) {
				$updraftplus->log('phpseclib3 failed loading signed key segment: '.$e->getMessage());
			}
		}

		if (!$loaded) {
			try {
				$key = call_user_func_array(array('phpseclib3_Crypt_PublicKeyLoader', 'load'), array($key));
			} catch (Exception $e) {
				$updraftplus->log('phpseclib3 failed loading full key: '.$e->getMessage());
			}
		}
	...
	/**
	 * Check whether or not the given server public host key is an instance of phpseclib3 elliptic curve algorithms and cryptographies
	 *
	 * @param mixed $public_host_key Server public hostkey
	 * @return boolean True if the given public host key is an instance of phpseclib3 elliptic_curve algorithms, false otherwise
	 */
...
class UpdraftPlus_ftp_wrapper {
	...
	public function put($local_file_path, $remote_file_path, $mode = FTP_BINARY, $resume = false, $updraftplus = false) {// phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- Unused parameters are for future use.
		...
		if ($this->curl_handle) {
			...
			if (version_compare(phpversion(), '5.3.0', '>=')) {
				curl_setopt($this->curl_handle, CURLOPT_PROGRESSFUNCTION, array($this, 'curl_progress_function'));// phpcs:ignore PHPCompatibility.Constants.NewConstants.curlopt_progressfunctionFound -- CURLOPT_PROGRESSFUNCTION is used intentionally as the minimum supported PHP version guarantees their availability.
	...
	public function get($local_file_path, $remote_file_path, $mode = FTP_BINARY, $resume = false, $updraftplus = false) {
		...
		if ($resume) {
			...
			clearstatcache($local_file_path);// phpcs:ignore PHPCompatibility.FunctionUse.NewFunctionParameters.clearstatcache_clear_realpath_cacheFound -- clearstatcache() parameter is used intentionally.
