...
// $nossl = $posted_settings['nossl'];
	...
	public function get_template_properties() {
		...
		if (preg_match('#^(https?://(\d+)\.(\d+)\.(\d+)\.(\d+))/#', apply_filters('updraftplus_gcloud_admin_page_url', UpdraftPlus_Options::admin_page_url()), $matches) && !$use_master) {
			...
			if (!$use_master) {
				$storage_google_instructions_label .= '<p><a href="https://teamupdraft.com/documentation/updraftplus/topics/cloud-storage/google-cloud/faqs/how-to-configure-google-cloud-storage-api-access-in-updraftplus/" target="_blank"><strong>'.__('For longer help, including screenshots, follow this link.', 'updraftplus').' '.__('The description below is sufficient for more expert users.', 'updraftplus').'</strong></a></p>';
