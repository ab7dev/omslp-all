...
use Borlabs\Cookie\Dto\Telemetry\MuPluginDto;
use Borlabs\Cookie\DtoList\System\KeyValueDtoList;
use Borlabs\Cookie\DtoList\Telemetry\DropInDtoList;
use Borlabs\Cookie\DtoList\Telemetry\MuPluginDtoList;
use DateTimeZone;
use WP_Error;
use WP_Post;
use WP_Screen;
...
final class WpFunction
{
    ...
    /**
     * @since 3.4.1 First time this was introduced in Borlabs Cookie.
     */
    public function getDropIns(): DropInDtoList
    {
        $dropInList = new DropInDtoList();
        $dropIns = get_dropins();

        foreach ($dropIns as $fileName => $dropIn) {
            $dropInDto = new DropInDto();
            $dropInDto->fileName = $fileName;
            $dropInDto->name = $dropIn['Name'] ?? null;
            $dropInDto->pluginUrl = $dropIn['PluginURI'] ?? null;
            $dropInDto->version = $dropIn['Version'] ?? null;
            $dropInList->add($dropInDto);
        }
     ...
     * @since 3.4.1 First time this was introduced in Borlabs Cookie.
     */
    public function getMuPlugins(): MuPluginDtoList
    {
        $muPluginList = new MuPluginDtoList();
        $muPlugins = get_mu_plugins();

        foreach ($muPlugins as $fileName => $muPlugin) {
            $muPluginDto = new MuPluginDto();
            $muPluginDto->fileName = $fileName;
            $muPluginDto->name = $muPlugin['Name'] ?? null;
            $muPluginDto->pluginUrl = $muPlugin['PluginURI'] ?? null;
            $muPluginDto->version = $muPlugin['Version'] ?? null;
            $muPluginList->add($muPluginDto);
        }
