...
final class FLThemeBuilderLoader {
	...
	static private function define_constants() {
		define( 'FL_THEME_BUILDER_VERSION', '1.5.2.4' );
		define( 'FL_THEME_BUILDER_FILE', trailingslashit( dirname( __DIR__, 1 ) ) . 'bb-theme-builder.php' );
	...
	static private function load_files() {
		...
		require_once FL_THEME_BUILDER_DIR . 'classes/class-fl-theme-builder-rules-custom-template.php';
	...
	static public function load_page_data_properties() {
		...
		require_once FL_THEME_BUILDER_DIR . 'classes/class-fl-page-data-term.php';
		...
		require_once FL_THEME_BUILDER_DIR . 'includes/page-data-term.php';
	}
	...
	static public function admin_notice() {
		...
		} elseif ( ( '{FL_BUILDER_VERSION}' !== FL_BUILDER_VERSION && version_compare( FL_BUILDER_VERSION, '2.8-alpha.1', '<' ) ) || ! class_exists( 'FLBuilderUserAccess' ) ) {
			$url = admin_url( 'plugins.php' );
			/* translators: %s: url */
			$message = __( 'Beaver Themer is only compatible with Beaver Builder 2.8 and above. Please <a href="%s">update</a> before continuing.', 'bb-theme-builder' );
	...
	static private function can_load() {
		if ( ! class_exists( 'FLBuilder' ) ) {
			...
			return false;
		} elseif ( '{FL_BUILDER_VERSION}' !== FL_BUILDER_VERSION && version_compare( FL_BUILDER_VERSION, '2.8-alpha.1', '<' ) ) {
			return false;
		} elseif ( version_compare( phpversion(), '5.3', '<' ) ) {
			return false;
		}
