...
final class FLPageDataPost {
	...
	static public function get_terms_list( $settings ) {
		...
		$settings = (object) wp_parse_args( $settings, $defaults );

		if ( isset( $settings->html_list ) && ( 'ul' === $settings->html_list || 'ol' === $settings->html_list ) ) {
			$separator  = $settings->html_list;
			$terms_list = self::get_the_term_list( $post->ID, $settings->taxonomy, "<$separator class='fl-{$settings->taxonomy}'><li>", '</li><li>', "</li></$separator>", $settings->linked, $settings->limit, $settings->display, $settings->orderby, $settings->order );
		} elseif ( isset( $settings->html_list ) && 'div' === $settings->html_list ) {
			$separator  = $settings->html_list;
			$terms_list = self::get_the_term_list( $post->ID, $settings->taxonomy, "<$separator class='fl-{$settings->taxonomy}'><span>", '</span><span>', "</span></$separator>", $settings->linked, $settings->limit, $settings->display, $settings->orderby, $settings->order );
		} else {
			$terms_list = self::get_the_term_list( $post->ID, $settings->taxonomy, '', $settings->separator, '', $settings->linked, $settings->limit, $settings->display, $settings->orderby, $settings->order );
	 ...
	 * @since 1.2.3
	 */
	static public function get_the_term_list( $id, $taxonomy, $before = '', $sep = '', $after = '', $linked = 'yes', $limit = false, $display = 'name', $orderby = 'name', $order = 'SORT_ASC' ) {
		...
		// set the flags as we can't convert a var to a php flag
		switch ( $order ) {
			case 'DESC':
			case 'SORT_DESC':
				$order = SORT_DESC;
		...
		foreach ( $terms as $term ) {
			$parts[] = isset( $term->{$orderby} ) ? $term->{$orderby} : null;
		}

		array_multisort( $parts, $order, $terms );
	...
	static public function get_custom_field( $settings ) {
		...
		if ( empty( $settings->key ) || ! isset( $post->ID ) ) {
			return '';
		}
		...
		if ( isset( $settings->format ) ) {
			$meta = gmdate( $settings->format, strtotime( $meta ) );
		}
		...
		if ( isset( $settings->value ) && '' !== $settings->value && isset( $settings->exp ) && '' !== $settings->exp ) {
			switch ( $settings->exp ) {
				...
				case 'contains':
					...
					if ( is_array( $meta ) ) {
						$meta = false !== array_search( trim( $settings->value ), $meta );
					}
					break;
			}
		} else {
