...
if ( defined( 'NFUPDATESDO' ) && NFUPDATESDO == 2 ) {
	...
	if ( empty( $nfw_options['lic'] ) ) { $nfw_options['lic'] = ''; }
...
if (NF_DISABLED) {
	echo '<div class="error notice is-dismissible"><p>' . __('Security rules cannot be updated when NinjaFirewall is disabled.', 'nfwplus') . '</p></div>';
	return;
}
...
if (! empty( $_POST['nfw_act'] ) ) {
	...
	if  ( isset( $_POST['check_updates'] ) ) {
		if ( $res = nf_sub_do_updates($update_url, $update_log, 0) ) {
			echo '<div class="updated notice is-dismissible"><p>' . __('Security rules have been updated.', 'nfwplus') . '</p></div>';
		} else {
			echo '<div class="updated notice is-dismissible"><p>' . __('No security rules update available.', 'nfwplus') . '</p></div>';
		}
...
if ( defined('DISABLE_WP_CRON') && DISABLE_WP_CRON == true ) {
	echo '<div class="notice-warning notice is-dismissible"><p>' . sprintf( __('It seems that %s is set. Ensure you have another way to run WP-Cron, otherwise NinjaFirewall automatic updates will not work.', 'nfwplus'), '<code>DISABLE_WP_CRON</code>' ) . '</p></div>';
	$cron_disabled = 1;
}
...
if ( empty($nfw_options['sched_updates']) || ! preg_match('/^[2-5]$/', $nfw_options['sched_updates']) ) {
	$sched_updates = 1;
} else {
...
<form method="post" name="fupdates">
	...
	<table class="form-table nfw-table">
		<tr style="background-color:#F9F9F9;border: solid 1px #DFDFDF;">
			<th scope="row" class="row-med"><?php _e('Automatically update NinjaFirewall security rules', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'danger', 'enable_updates', __('Enabled', 'nfwplus'), __('Disabled', 'nfwplus'), 'large', $enable_updates, false, 'onclick="nfwjs_up_down(\'upd_table\');"' ) ?>
			</td>
		</tr>
	</table>
	...
	<div id="upd_table"<?php echo $enable_updates == 1 ? '' : ' style="display:none"' ?>>
		<table class="form-table nfw-table">
			<tr>
				<th scope="row" class="row-med"><?php _e('Check for updates', 'nfwplus') ?></th>
					<td>
						<select name="sched_updates">
							<option value="5"<?php selected($sched_updates, 5) ?>><?php _e('Every 15 minutes', 'nfwplus') ?></option>
							<option value="4"<?php selected($sched_updates, 4) ?>><?php _e('Every 30 minutes', 'nfwplus') ?></option>
							<option value="1"<?php selected($sched_updates, 1) ?>><?php _e('Hourly', 'nfwplus') ?></option>
							<option value="2"<?php selected($sched_updates, 2) ?>><?php _e('Twicedaily', 'nfwplus') ?></option>
							<option value="3"<?php selected($sched_updates, 3) ?>><?php _e('Daily', 'nfwplus') ?></option>
						</select>
						<?php
						if ( $nextcron = wp_next_scheduled('nfsecupdates') ) {
							...
							?>
								<p class="description" style="color:red"><?php _e('The next scheduled date is in the past! WordPress wp-cron may not be working, may have been disabled or is currently running. Try to reload this page in a few seconds.', 'nfwplus'); ?></p>
							<?php
								}
							} else {
							?>
								<p class="description"><?php printf( __('Next scheduled update will start in approximately %s day, %s hour(s), %s minute(s) and %s seconds.', 'nfwplus'), $diff->format('%a') % 7, $diff->format('%h'), $diff->format('%i'), $diff->format('%s') ) ?></p>
							<?php
							}
						}
						?>
			...
			<tr>
				<th scope="row" class="row-med"><?php _e('Notification', 'nfwplus') ?></th>
				<td>
					<p><label><input type="checkbox" name="notify_updates" value="1"<?php checked($notify_updates, 1) ?> /><?php _e('Send me a report by email when security rules have been updated.', 'nfwplus') ?></label></p>
					<p class="description"><?php _e('Reports will be sent to the contact email address defined in the Event Notifications menu.', 'nfwplus') ?></p>
				</td>
			</tr>
			...
			if (! empty($nfw_options['enable_updates']) || ! empty($tmp_showlog) ) {
				...
				if ( file_exists($update_log) ) {
					...
					$log_data[] = __('The updates log is currently empty.', 'nfwplus');
				}
			?>
			<tr>
				<th scope="row" class="row-med"><?php _e('Updates Log', 'nfwplus') ?></th>
				<td>
					...
					if ( $count == 0 ) {
						_e('The updates log is currently empty.', 'nfwplus');
					}
					?></textarea>
					<p class="description"><?php _e('The log is deleted automatically.', 'nfwplus') ?></p>
				</td>
				</tr>
				<?php
			}
	...
	<p>
		...
		<input name="save_options" type="submit" class="button-primary" value="<?php _e('Save Updates Options', 'nfwplus') ?>" />&nbsp;&nbsp;&nbsp;&nbsp;
		<input name="check_updates" type="submit" class="button-secondary" value="<?php _e('Check For Updates Now!', 'nfwplus') ?>" />&nbsp;&nbsp;&nbsp;&nbsp;
		<?php
		...
		?>
		<input name="clear_log" type="submit" value="<?php _e('Delete Log', 'nfwplus') ?>" class="button-secondary"<?php echo $style ?> />
	</p>
	</form>
<?php
...
function nf_sub_updates_save() {
	...
	if ( empty($_POST['sched_updates']) || ! preg_match('/^[2-5]$/', $_POST['sched_updates']) ) {
...
function nf_sub_do_updates($update_url, $update_log, $NFUPDATESDO = 1) {
	...
	if (! preg_match('/^a:\d+:{i:\d/', $data ) ) {
		nf_sub_updates_log(
			$update_log,
			__('Error: Wrong rules format.', 'nfwplus')
		);
		return 0;
	}
	...
	if (! is_wp_error($res) ) {
		if ( $res['response']['code'] == 200 ) {
			...
			if ( $new_version[0] != 3 ) {
				...
				nf_sub_updates_log(
					$update_log,
					sprintf( __('Error: %s', 'nfwplus'), $new_version[1] )
				);
				return 0;
			}
			...
			if (! preg_match('/^\d{8}\.\d+$/', $new_version[1]) ) {
				...
				nf_sub_updates_log(
					$update_log,
					__('Error: Unable to retrieve the new rules version.', 'nfwplus')
				);
				return 0;
			}
			// Compare versions:
			if ( version_compare($rules_version, $new_version[1], '<') ) {
				...
				nf_sub_updates_log(
				$update_log,
				__('No security rules update available.', 'nfwplus')
				);
			}
		// Not a 200 OK ret code :
			...
			nf_sub_updates_log(
				$update_log,
				sprintf( __('Error: Server returned a %s HTTP error code (#1).', 'nfwplus'), htmlspecialchars($res['response']['code']))
			);
		}
	// Connection error :
	} else {
		nf_sub_updates_log(
			$update_log,
			__('Error: Unable to connect to the remote server', 'nfwplus') . htmlspecialchars(" ({$res->get_error_message()})")
		);
	}
	return 0;
}
...
function nf_sub_updates_download($update_url, $update_log, $new_rules_version) {
	...
	if (! is_wp_error($res) ) {
		if ( $res['response']['code'] == 200 ) {
			...
			if ( function_exists( 'openssl_pkey_get_public') && function_exists( 'openssl_verify' ) && defined('OPENSSL_ALGO_SHA256') ) {
				...
				if ( $verify != 1 ) {
					nf_sub_updates_log(
						$update_log,
						sprintf( __('Error: The new rules %s digital signature is not correct. Aborting update, rules may have been tampered with.', 'nfwplus'), htmlspecialchars($data[0]) )
					);
					return 0;
				}
			}
		...
		// Not a 200 OK ret code :
			...
			nf_sub_updates_log(
				$update_log,
				sprintf( __('Error: Server returned a %s HTTP error code (#2).', 'nfwplus'), htmlspecialchars($res['response']['code']))
			);
		}
	// Connection error :
	} else {
