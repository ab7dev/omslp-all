...
function nfw_contextual_help() {
	echo '<div style="text-align:right;font-weight:normal;padding-top: 9px;">' .
		...
		esc_html('Click on the above "Help" tab for help.', 'nfwplus') .
		'</span></div>';
}
...
// =====================================================================
// Check whether the user is whitelisted ( .htninja, IP Access Control).
...
function nf_wp_insert_post_empty_content( $maybe_empty, $postarr ) {
	...
	if (! empty( $postarr['post_type'] ) &&
		...
		if ( empty( $id ) ) {
			...
			$action = sprintf( __('Attempt to create a new %s', 'nfwplus'), $postarr['post_type'] );
		/**
		 ...
		 */
			...
			$action = sprintf(
				__('Attempt to edit a published %s (ID: %s)', 'nfwplus'), $postarr['post_type'], $id
				);
		}
		...
		if ( empty( $current_user->user_login ) ) {
			$user = __('Unauthenticated user', 'nfwplus');
		} else {
			$user = $current_user->user_login;
		}

		$subject = __('Blocked post/page edition attempt', 'nfwplus');
		...
		wp_die(
			'NinjaFirewall: '. __('You are not allowed to perform this task.', 'nfwplus'),
			'NinjaFirewall: '. __('You are not allowed to perform this task.', 'nfwplus'),
...
function nf_pre_delete_post( $delete, $post, $force_delete ) {
	...
	if ( ( $post->post_type == 'post' ||
		...
		if (! current_user_can( "delete_{$post->post_type}", $post->ID ) ) {
			...
			$subject = __('Blocked post/page deletion attempt', 'nfwplus');
...
function nfw_account_creation( $user_login ) {
	...
	$subject = __('Blocked user account creation', 'nfwplus');
	/**
	 * Write to log.
	 */
	NinjaFirewall_log::write(
...
function nfw_delete_user( $user_id ) {
	...
	$subject = __('Blocked user deletion attempt', 'nfwplus');
	/**
	 * Write to log.
	 */
	NinjaFirewall_log::write(
...
function nfw_garbage_collector() {
	...
	if ( nfw_validate_option( $nfw_options ) === false ) {
		...
		while ( is_array( $files ) && ! empty( $files[0] ) ) {
			...
			// Delete this corrupted backup file
				...
				nfw_log_error(
					sprintf( __('Backup file is corrupted, deleting it (%s)', 'nfwplus'), $last_file )
				);
				unlink( $last_file );
			}
		}
		...
		if (! empty( $valid_option ) ) {
			...
			nfw_log_error( sprintf( __('NinjaFirewall\'s options are corrupted, restoring them from '.
				'last known good backup file (%s)', 'nfwplus'), $last_file ) );
	...
	$list = NinjaFirewall_helpers::nfw_glob( $path, 'rl\..+?\.php$', true, true );
	foreach( $list as $file ) {
	...
	if (! empty( $nfw_options['load_i18n'] ) ) {
		$lcupdate = 1;
		$lc = get_user_locale();
		if ( file_exists( WP_LANG_DIR ."/plugins/nfwplus-{$lc}.mo" ) ) {
			// Don't update more often than every 55mn
			$nfw_ctime = filectime( WP_LANG_DIR ."/plugins/nfwplus-{$lc}.mo" );
			if ( $now - $nfw_ctime < 3300 ) {
				$lcupdate = 0;
			}
		}
		if ( $lcupdate ) {
...
// $roles can be a string or an array (e.g., $current_user->roles).

function nfw_is_goodguy( $roles ) {

	$nfw_options = nfw_get_option('nfw_options');
	// No one is whitelisted?
	if ( empty( $nfw_options['ac_roles'] ) || $nfw_options['ac_roles'] == '|') {
		return;
	}

	// We try to find the user roles if we don't know it yet
	if ( empty( $roles ) ) {
		$current_user = wp_get_current_user();
		$roles = array();
	...
	if ( is_array( $roles ) ) {
		foreach( $roles as $k => $v ) {
			if (! empty( $v ) && strpos( $nfw_options['ac_roles'], "|{$v}|" ) !== false ) {
				return $v;
			}
		}

	} elseif ( strpos( $nfw_options['ac_roles'], "|{$roles}|" ) !== false ) {
...
function nfw_send_loginemail( $user_login, $whoami ) {
	...
	/**
	 * We send an email only if the user is not in the IP whitelist
	 * and the "Do not send a notification if the user is in the
	 * IP Access Control whitelist" checkbox is enabled.
	 */
	if ( nfw_is_whitelisted() && ! empty( $nfw_options['a_01'] ) ) {
		/**
		 * Let it go.
		 */
		return;
...
function nfw_admin_notice() {
	...
	if (! empty( $nfw_options['lic'] ) ) {
		if (! empty( $nfw_options['lic_exp'] ) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $nfw_options['lic_exp'] ) ) {
			...
			__('Your NinaFirewall license has expired!', 'nfwplus') . ' <a href="admin.php?page=NinjaFirewall&tab=license">' .
			__('Click here to renew it', 'nfwplus') . '</a>.</p></div>';
			} elseif ( $nfw_options['lic_exp'] < date('Y-m-d', strtotime("+30 day")) ){
				echo '<div class="error notice is-dismissible"><p>' .
			__('Your NinaFirewall license will expire soon!', 'nfwplus') . ' <a href="admin.php?page=NinjaFirewall&tab=license">' .
			__('Click here to renew it', 'nfwplus') . '</a>.</p></div>';
			}
		} else {
			echo '<div class="error notice is-dismissible"><p>' .
			__('You do not have a valid NinjaFirewall license', 'nfwplus') . ' (#2)! <a href="admin.php?page=NinjaFirewall&tab=license">' .
	...
	if (! file_exists(NFW_LOG_DIR . '/nfwlog') ) {
		echo '<div class="error notice is-dismissible"><p><strong>' . __('NinjaFirewall error', 'nfwplus') . ' :</strong> ' .
			sprintf( __('%s directory cannot be created. Please review your installation and ensure that %s is writable.', 'nfwplus'), '<code>'. esc_html(NFW_LOG_DIR) .'/nfwlog/</code>',  '<code>/wp-content/</code>') . '</p></div>';
	}
	if (! is_writable(NFW_LOG_DIR . '/nfwlog') ) {
		echo '<div class="error notice is-dismissible"><p><strong>' . __('NinjaFirewall error', 'nfwplus') . ' :</strong> ' .
			sprintf( __('%s directory is read-only. Please review your installation and ensure that %s is writable.', 'nfwplus'), '<code>'. esc_html(NFW_LOG_DIR) .'/nfwlog/</code>', '<code>/nfwlog/</code>') . '</p></div>';
	}
...
function nfwhook_rest_authentication_errors( $res ) {
	...
	if (! empty( $nfw_options['no_restapi'] ) ) {
		...
		return new WP_Error(
			...
			esc_html__('Forbidden access', 'nfwplus'),
	...
	if ( is_wp_error( $user ) ) {
		if ( preg_match('/^(?:in(?:correct_password|valid_(?:username|email))|authentication_failed)$/', $user->get_error_code() ) ) {
			...
			$user = new WP_Error('denied',
				__('Invalid username, email address or password.', 'nfwplus') .
				"<br /><a href=\"$lostpass\">".
				__('Lost your password?', 'nfwplus').
				'</a>'
			);
...
function nf_check_dbdata() {
	...
	if ( $old_hash == md5( serialize( $adm_users ) ) ) {
		...
		if (! empty( $nfw_options['a_41'] ) ) {
			NinjaFirewall_log::write(
				__('Database changes detected', 'nfwplus'),
				__('administrator account', 'nfwplus'),
...
function nfw_in_plugin_update_message( $plugin_data, $r ) {
	...
	echo esc_html__('Important: NinjaFirewall has detected that this is a security update.', 'nfwplus') . ' ' .
	esc_html__("Don't leave your blog at risk, make sure to update as soon as possible.", 'nfwplus')  . ' ' .
	...
	esc_html__('More info about this warning.', 'nfwplus') .
	'</a></span>';
}
...
function nfw_debug_backtrace( $nfw_options ) {
	...
	if ( $verbosity !== false ) {
		...
		$return['message'] = __('A PHP backtrace has been attached to this message for your convenience.', 'nfwplus') . "\n\n";
	}
	return $return;
}
...
function nfw_enable_wpwaf() {
	...
	if (! is_dir( WPMU_PLUGIN_DIR ) ) {
		if (! @mkdir( WPMU_PLUGIN_DIR, 0755, true ) ) {
			return sprintf(
				esc_html__('Error, cannot create the %s folder.', 'nfwplus') .' '.
				esc_html__('Check your server permissions and try again.', 'nfwplus'),
	...
	if (! is_writable( WPMU_PLUGIN_DIR ) ) {
		return sprintf(
			esc_html__('Error, the %s folder is not writable.', 'nfwplus') .' '.
			esc_html__('Check your server permissions and try again.', 'nfwplus'),
			esc_html( WPMU_PLUGIN_DIR )
		);
	}
	...
	if (! file_exists( WPMU_PLUGIN_DIR .'/'. NINJAFIREWALL_MU_PLUGIN ) ) {
		return sprintf(
			esc_html__('Error, cannot write %s.', 'nfwplus') .' '.
			esc_html__('Check your server permissions and try again.', 'nfwplus'),
