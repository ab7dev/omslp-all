...
?>
<div class="wrap">
	<h1><img style="vertical-align:top;width:33px;height:33px;" src="<?php echo plugins_url( '/nfwplus/images/ninjafirewall_32.png' ) ?>">&nbsp;<?php _e('Firewall Policies', 'nfwplus') ?></h1>
<?php
...
<h2 class="nav-tab-wrapper wp-clearfix" style="cursor:pointer">
	<a id="tab-basic" class="nav-tab<?php echo $basic_tab ?>" onClick="nfwjs_switch_tabs('basic', 'basic:intermediate:advanced')"><?php _e( 'Basic Policies', 'nfwplus' ) ?></a>
	<a id="tab-intermediate" class="nav-tab<?php echo $intermediate_tab ?>" onClick="nfwjs_switch_tabs('intermediate', 'basic:intermediate:advanced')"><?php _e( 'Intermediate Policies', 'nfwplus' ) ?></a>
	<a id="tab-advanced" class="nav-tab<?php echo $advanced_tab ?>" onClick="nfwjs_switch_tabs('advanced', 'basic:intermediate:advanced')"><?php _e( 'Advanced Policies', 'nfwplus' ) ?></a>
...
<div id="basic-options"<?php echo $basic_div ?>>
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('Enable NinjaFirewall for', 'nfwplus') ?></th>
			<td>
	...
	if ( empty( $nfw_options['scan_zip'] ) ) {
		$scan_zip = 0;
	} else {
		if ( $nfw_options['uploads'] == 2 ) {
			$scan_zip = 1;
		} else {
			$scan_zip = 0;
		}
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('File Uploads', 'nfwplus') ?></th>
			<td>
				<p><label><input type="radio" name="nfw_options[uploads]"<?php checked( $uploads, 0 ) ?> value="0" id="uf1" onClick="nfwjs_upload_onoff(0);">&nbsp;<?php _e('Disallow uploads', 'nfwplus') ?></label></p>

				<p><label><input type="radio" name="nfw_options[uploads]"<?php checked( $uploads, 1 ) ?> value="1" id="uf1" onClick="nfwjs_upload_onoff(1);">&nbsp;<?php _e('Allow uploads', 'nfwplus') ?></label></p>

				<p><label><input type="radio" name="nfw_options[uploads]"<?php checked( $uploads, 2 ) ?> value="2" id="uf2" onClick="nfwjs_upload_onoff(2);">&nbsp;<?php echo __('Allow, but block dangerous files', 'nfwplus') .' '. __('(default)', 'nfwplus') ?></label></p>
				<p style="margin-left:20px" class="description"><?php _e('See contextual help for the list of files.', 'nfwplus') ?></p>
				<?php
				if ( $uploads != 2 ) {
					$disabled = " disabled";
				} else {
				...
				?>
				<p style="margin-left:20px"><label><input id="scanzip"<?php echo $disabled ?> type="checkbox" name="nfw_options[scan_zip]" value="1"<?php checked( $scan_zip, 1 ) ?>>&nbsp;<?php _e('Apply to ZIP archives file contents', 'nfwplus') ?></label></p>
				<br />
				<p><label><input id="san" type="checkbox" onclick='return nfwjs_sanitise(this);' name="nfw_options[sanitise_fn]"<?php checked( $sanitise_fn, 1 ); disabled( $uploads, 0 ) ?>>&nbsp;<?php _e('Sanitise filenames', 'nfwplus') ?>
				...
				<p><?php _e('Maximum allowed file size', 'nfwplus' ) ?>&nbsp;<input id="sizeid" class="medium-text" type="number" name="nfw_options[upload_maxsize]"<?php disabled( $uploads, 0 ) ?> size="5" min="1" value="<?php echo $upload_maxsize ?>" />&nbsp;KB</p>
			</td>
		</tr>
	</table>
	...
	// We must disable these options if their constants are already defined
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('Block direct access to any PHP file located in one of these directories', 'nfwplus'); ?></th>
			<td>
				<?php
				if ( defined('NFW_WPWAF') ) {
					echo $full_waf_msg;
				...
				<table class="form-table">
					...
					<tr style="border: solid 1px #DFDFDF;">
						...
						<br />
						<p class="description"><?php _e('NinjaFirewall will not block access to the TinyMCE WYSIWYG editor even if this option is enabled.', 'nfwplus') ?></p>
						</td>
					</tr>
					<tr style="border: solid 1px #DFDFDF;">
					...
					<tr style="border: solid 1px #DFDFDF;">
						...
						<br />
						<br />
						<p class="description"><?php _e('Unless you have PHP scripts in a "/cache/" folder that need to be accessed by your visitors, we recommend to enable this option.', 'nfwplus') ?></p>
						</td>
					</tr>
				</table>
				<br />&nbsp;
		...
		if ( defined('NFW_DISABLE_PRVESC2') ) {
			$msg = '<p class="description">'. sprintf( __('To enable this option, please remove the %s constant from your wp-config.php or .htninja script.', 'nfwplus'), '<code>NFW_DISABLE_PRVESC2</code>' ) .'</p>';
			$disabled = 1;
			$disabled_mu = 1;
			$disallow_privesc = 0;
			$disallow_privesc_mu = 0;
		...
		if ( is_multisite() ) {
			...
			$main_site = ' '.__('(main site only)', 'nfwplus');
			$network_only = ' '.__('(not recommended if you allow sites registration)', 'nfwplus');
		// Single site
		} else {
			...
			$disabled_mu = 1;
			$main_site = '';
			$network_only = ' '.__('(multisite installation only)', 'nfwplus');
		}
		...
		?>
		<tr>
			<th scope="row" class="row-med"><?php _e('Permissions', 'nfwplus') ?></th>
			<td>
				<p><label><input type="checkbox" name="nfw_options[disallow_settings]" value="1"<?php checked( $disallow_settings, 1 ) ?>>&nbsp;<?php echo _e('Block attempts to modify important WordPress settings', 'nfwplus') ?></label></p>
				<p><label><input<?php echo $reg_user ?> type="checkbox" name="nfw_options[disallow_creation]" value="1"<?php checked( $disallow_creation, 1 ) ?>>&nbsp;<?php _e('Block user accounts creation', 'nfwplus') ?></label></p>
				<p><label><input type="checkbox" name="nfw_options[disallow_deletion]" value="1"<?php checked( $disallow_deletion, 1 ) ?>>&nbsp;<?php _e('Block user accounts deletion', 'nfwplus') ?></label></p>
				<p><label><input <?php disabled( $disabled, 1 ) ?>type="checkbox" name="nfw_options[disallow_privesc]" value="1"<?php checked( $disallow_privesc, 1 ) ?>>&nbsp;<?php _e('Block attempts to gain administrative privileges', 'nfwplus'); echo $main_site; ?></label></p>
				<?php echo $msg ?>
				<p style="margin-left:20px"><label><input<?php echo $reg_site ?> <?php disabled( $disabled_mu, 1 ) ?>type="checkbox" name="nfw_options[disallow_privesc_mu]" value="1"<?php checked( $disallow_privesc_mu, 1 ) ?>>&nbsp;<?php _e('Apply to all subsites in the network', 'nfwplus'); echo $network_only; ?></label></p>

				<p><label><input type="checkbox" name="nfw_options[disallow_publish]" value="1"<?php checked( $disallow_publish, 1 ) ?>>&nbsp;<?php echo _e('Block attempts to publish, edit or delete a published post by users who do not have the right capabilities', 'nfwplus') ?> <?php _e('(recommended)', 'nfwplus') ?></label></p>
			</td>
		</tr>
		...
		?>
		<tr>
			<th scope="row" class="row-med"><?php _e('WordPress AJAX', 'nfwplus') ?></th>
			<td>
				<p><label><input type="checkbox" name="nfw_options[admin_ajax]" value="1"<?php checked( $admin_ajax, 1 ) ?>>&nbsp;<?php _e('Protect <code>admin-ajax.php</code> against bots', 'nfwplus') ?></label></p>
				<p class="description"><?php printf( __('Your server IP (%s), localhost and private IP addresses will not be affected by this policy.', 'nfwplus'), htmlspecialchars( $_SERVER['SERVER_ADDR'] ) ) ?></p>
			</td>
		</tr>

		<tr>
			<th scope="row" class="row-med"><?php _e('Protect against username enumeration', 'nfwplus') ?></th>
			<td>
				...
				<p><label><input type="checkbox" name="nfw_options[enum_sitemap]" value="1"<?php checked( $enum_sitemap, 1 ) ?>>&nbsp;<?php _e('Through the author sitemap', 'nfwplus') ?></label></p>
				...
				<p><label><input type="checkbox" name="nfw_options[enum_feed]" value="1"<?php checked( $enum_feed, 1 ) ?>>&nbsp;<?php _e('Through the blog feed', 'nfwplus') ?></label></p>
				<p><label><input type="checkbox" name="nfw_options[enum_restapi]" value="1"<?php checked( $enum_restapi, 1 ) ?>>&nbsp;<?php _e('Through the WordPress REST API', 'nfwplus') ?></label> <font color="red">*</font></p>
			</td>
		</tr>

		<tr>
			<th scope="row" class="row-med"><?php _e('WordPress REST API', 'nfwplus') ?></th>
			<td>
				<p><label><input type="checkbox" onClick="nfwjs_no_restapi(this);" name="nfw_options[no_restapi]" value="1"<?php checked( $no_restapi, 1 ) ?>>&nbsp;<?php _e('Block any access to the API', 'nfwplus') ?></label> <font color="red">*</font></p>
				<p style="margin-left:20px"><label><input type="checkbox"<?php disabled($no_restapi, 0) ?> name="nfw_options[restapi_loggedin]" value="1"<?php checked( $restapi_loggedin, 1 ) ?>>&nbsp;<?php _e('Allow logged-in users to access the API', 'nfwplus') ?></label></p>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php _e('WordPress XML-RPC API', 'nfwplus') ?></th>
			<td>
				<p><label><input type="checkbox" onClick="nfwjs_no_xmlrpc(this);" name="nfw_options[no_xmlrpc]" value="1"<?php checked( $no_xmlrpc, 1 ) ?>>&nbsp;<?php _e('Block any access to the API', 'nfwplus') ?></label> <font color="red">*</font></p>
		...
		<tr>
			<th scope="row" class="row-med"><?php _e('Application Passwords', 'nfwplus') ?></th>
			<td>
				<p><label><input type="checkbox" name="nfw_options[no_appswd]" value="1"<?php checked( $no_appswd, 1 ) ?>>&nbsp;<?php printf( __('Disable <a %s>Application Passwords</a>', 'nfwplus'), 'href="https://make.wordpress.org/core/2020/11/05/application-passwords-integration-guide/" target="_blank" rel="noreferrer noopener"') ?></label></p>
			</td>
		</tr>

		<tr valign="top">
			<th scope="row" class="row-med" style="vertical-align:top"><?php _e('Block <code>POST</code> requests in the themes folder', 'nfwplus') ?> <code>/<?php echo basename(WP_CONTENT_DIR); ?>/themes</code></th>
			<td>
		...
		<tr valign="top">
			<th scope="row" class="row-med"><a name="builtinconstants"></a><?php _e('Force HTTPS for admin and logins', 'nfwplus') ?> <code><a href="https://wordpress.org/support/article/editing-wp-config-php/#require-ssl-for-admin-and-logins" target="_blank" rel="noreferrer noopener">FORCE_SSL_ADMIN</a></code></th>
			<td>
		...
		<tr valign="top">
			<th scope="row" class="row-med"><?php _e('Disable the plugin and theme editor', 'nfwplus') ?> <code><a href="https://wordpress.org/support/article/editing-wp-config-php/#disable-the-plugin-and-theme-editor" target="_blank" rel="noreferrer noopener">DISALLOW_FILE_EDIT</a></code></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[disallow_edit]', $yes, $no, 'small', $disallow_edit, $disallow_edit_already_enabled ) ?>
			</td>
		</tr>
		<tr valign="top">
			<th scope="row" class="row-med"><?php _e('Disable plugin and theme update/installation', 'nfwplus') ?> <code><a href="https://wordpress.org/support/article/editing-wp-config-php/#disable-plugin-and-theme-update-and-installation" target="_blank" rel="noreferrer noopener">DISALLOW_FILE_MODS</a></code></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[disallow_mods]', $yes, $no, 'small', $disallow_mods, $disallow_mods_already_enabled ) ?>
			</td>
		</tr>
		<tr valign="top">
			<th scope="row" class="row-med"><?php _e('Disable the fatal error handler', 'nfwplus') ?> <code><a href="https://make.wordpress.org/core/2019/01/14/php-site-health-mechanisms-in-5-1/" target="_blank" rel="noreferrer noopener">WP_DISABLE_FATAL_ERROR_HANDLER</a></code></th>
			<td>
...
<div id="intermediate-options"<?php echo $intermediate_div ?>>
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('Scan <code>GET</code> variable', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[get_scan]', $yes, $no, 'small', $get_scan ) ?>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php _e('Sanitise <code>GET</code> variable', 'nfwplus') ?></th>
			<td>
	...
	<table class="form-table nfw-table">
		<tr valign="top">
			<th scope="row" class="row-med"><?php _e('Scan <code>POST</code> variable', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[post_scan]', $yes, $no, 'small', $post_scan ) ?>
			</td>
		</tr>
		<tr valign="top">
			<th scope="row" class="row-med"><?php _e('Sanitise <code>POST</code> variable', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'warning', 'nfw_options[post_sanitise]', $yes, $no, 'small', $post_sanitise ) ?>
				<p class="description">&nbsp;<?php _e('Do not enable this option unless you know what you are doing!', 'nfwplus') ?></p>
			</td>
		</tr>
		<tr valign="top">
			<th scope="row" class="row-med"><?php _e('Decode Base64-encoded <code>POST</code> variable', 'nfwplus') ?></th>
			<td>
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('Sanitise <code>REQUEST</code> variable', 'nfwplus') ?></th>
			<td>
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('Scan cookies', 'nfwplus') ?></th>
			<td>
				 <?php nfw_toggle_switch( 'info', 'nfw_options[cookies_scan]', $yes, $no, 'small', $cookies_scan ) ?>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php _e('Sanitise cookies', 'nfwplus') ?></th>
			<td>
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('Scan <code>HTTP_USER_AGENT</code>', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[ua_scan]', $yes, $no, 'small', $ua_scan ) ?>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php _e('Sanitise <code>HTTP_USER_AGENT</code>', 'nfwplus') ?></th>
			<td>
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('Scan <code>HTTP_REFERER</code>', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[referer_scan]', $yes, $no, 'small', $referer_scan ) ?>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php _e('Sanitise <code>HTTP_REFERER</code>', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[referer_sanitise]', $yes, $no, 'small', $referer_sanitise ) ?>
			</td>
		</tr>
		<tr valign="top">
			<th scope="row" class="row-med"><?php _e('Block <code>POST</code> requests that do not have an <code>HTTP_REFERER</code> header', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[referer_post]', $yes, $no, 'small', $referer_post ) ?>
				<p class="description">&nbsp;<?php _e('Keep this option disabled if you are using scripts such as Paypal IPN or WordPress WP-Cron, unless you added their IP address or URL to your Access Control whitelist.', 'nfwplus') ?></p>
			</td>
		</tr>
	</table>
...
<div id="advanced-options"<?php echo $advanced_div ?>>
	...
	$tpl = __('The "HTTP response headers" options below are disabled because the %s PHP function is not available on your server.', 'nfwplus');
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php printf( __('Set %s to protect against MIME type confusion attacks', 'nfwplus'), '<a href="https://blog.nintechnet.com/securing-wordpress-with-a-web-application-firewall-ninjafirewall/#advanced-policies" target="_blank" rel="noreferrer noopener">X-Content-Type-Options</a>') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[x_content_type_options]', $yes, $no, 'small', $nfw_options['response_headers'][1], $err ) ?>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php printf( __('Set %s to protect against clickjacking attempts', 'nfwplus'), '<a href="https://blog.nintechnet.com/securing-wordpress-with-a-web-application-firewall-ninjafirewall/#advanced-policies" target="_blank" rel="noreferrer noopener">X-Frame-Options</a>') ?></th>
			<td>
				...
				<p class="description"><?php _e('Setting this option to <code>DENY</code> may break some functionality on your blog, its themes or plugins.', 'nfwplus') ?></p>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php printf( __('Set %s', 'nfwplus'), '<a href="https://blog.nintechnet.com/securing-wordpress-with-a-web-application-firewall-ninjafirewall/#advanced-policies" target="_blank" rel="noreferrer noopener">X-XSS-Protection</a>') ?></th>
			<td>
				<select name="nfw_options[x_xss_protection]" <?php disabled( $err, 1 ) ?>>
					...
					<option value="0"<?php selected( $nfw_options['response_headers'][3], 0 ) ?>><?php printf( __('Set to %s', 'nfwplus'), '"0"'); ?></option>
					<option value="2"<?php selected( $nfw_options['response_headers'][3], 2 ) ?>><?php printf( __('Set to %s', 'nfwplus'), '"1"'); ?></option>
					<option value="1"<?php selected( $nfw_options['response_headers'][3], 1 ) ?>><?php printf( __('Set to %s', 'nfwplus'), '"1; mode=block"') ?></option>
				</select>
				<p class="description"><span class="dashicons dashicons-warning nfw-warning"></span><?php esc_html_e('This header is deprecated and most browsers phased out support for it. Consider using Content-Security-Policy instead.', 'nfwplus') ?></p>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php printf( __('Force %s flag on all cookies to mitigate CSRF attacks', 'nfwplus'), '<a href="https://blog.nintechnet.com/securing-wordpress-with-a-web-application-firewall-ninjafirewall/#advanced-policies" target="_blank" rel="noreferrer noopener">SameSite</a>' ) ?></th>
			<td>
		...
		<tr>
			<th scope="row" class="row-med"><?php printf( __('Force %s flag on all cookies to mitigate XSS attacks', 'nfwplus'), '<a href="https://blog.nintechnet.com/securing-wordpress-with-a-web-application-firewall-ninjafirewall/#advanced-policies" target="_blank" rel="noreferrer noopener">HttpOnly</a>') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[cookies_httponly]', $yes, $no, 'small', $nfw_options['response_headers'][0], $err ) ?>
				<p class="description"><?php _e('If your PHP scripts use cookies that need to be accessed from JavaScript, you should not enable this option.', 'nfwplus') ?></p>
			</td>
		</tr>
		<?php
		...
		if ( NFW_IS_HTTPS == false && ! $err ) {
			$hsts_err = 1;
			$hsts_msg = __('HSTS headers can only be set when you are accessing your site over HTTPS.', 'nfwplus');
		} else {
			$hsts_msg = '';
			$hsts_err = 0;
		}
		...
		?>
		<tr>
			<th scope="row" class="row-med"><?php printf( __('Set %s (HSTS) to enforce secure connections to the server', 'nfwplus'), '<a href="https://blog.nintechnet.com/securing-wordpress-with-a-web-application-firewall-ninjafirewall/#advanced-policies" target="_blank" rel="noreferrer noopener">Strict-Transport-Security</a>') ?></th>
			<td>
				<select name="nfw_options[strict_transport]" <?php disabled( $hsts_err, 1 ) ?>>
					...
					<option value="4"<?php selected( $nfw_options['response_headers'][4], 4 ) ?>><?php _e('Set "max-age" to 0', 'nfwplus') ?></option>
					<option value="1"<?php selected( $nfw_options['response_headers'][4], 1 ) ?>><?php _e('1 month', 'nfwplus') ?></option>
					<option value="2"<?php selected( $nfw_options['response_headers'][4], 2 ) ?>><?php _e('6 months', 'nfwplus') ?></option>
					<option value="3"<?php selected( $nfw_options['response_headers'][4], 3 ) ?>><?php _e('1 year', 'nfwplus') ?></option>
					<option value="5"<?php selected( $nfw_options['response_headers'][4], 5 ) ?>><?php _e('2 years', 'nfwplus') ?></option>
				</select>
				<?php
				...
				?>
				<p><label><input type="checkbox" name="nfw_options[strict_transport_sub]" value="1"<?php checked( $subdom, 1 );disabled($hsts_err, 1) ?>>&nbsp;<?php _e('Apply to subdomains', 'nfwplus') ?></label>&nbsp;&nbsp;<label><input type="checkbox" name="nfw_options[strict_transport_preload]" value="1"<?php checked( $preload, 1 );disabled($hsts_err, 1) ?>>&nbsp;<?php _e('Preload', 'nfwplus') ?></label></p>
				<?php
				if (! empty( $hsts_msg ) ) {
		...
		?>
		<tr>
			<th scope="row" class="row-med"><?php printf( __('Set %s for the website frontend', 'nfwplus'), '<a href="https://blog.nintechnet.com/securing-wordpress-with-a-web-application-firewall-ninjafirewall/#advanced-policies" target="_blank" rel="noreferrer noopener">Content-Security-Policy</a>') ?></th>
			<td>
				...
				<p class="description"><?php _e('This CSP header will apply to the website frontend only.', 'nfwplus') ?></p>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php printf( __('Set %s for the WordPress admin dashboard', 'nfwplus'), '<a href="https://blog.nintechnet.com/securing-wordpress-with-a-web-application-firewall-ninjafirewall/#advanced-policies" target="_blank" rel="noreferrer noopener">Content-Security-Policy</a>') ?></th>
			<td>
				...
				<p class="description"><?php _e('This CSP header will apply to the WordPress admin dashboard only.', 'nfwplus') ?></p>
				<?php echo $err_msg ?>
			</td>
		</tr>
		...
		?>
		<tr>
			<th scope="row"><?php printf( __("Set %s (Chrome, Opera and Firefox browsers)", 'nfwplus'), '<a href="https://blog.nintechnet.com/securing-wordpress-with-a-web-application-firewall-ninjafirewall/#advanced-policies" target="_blank" rel="noreferrer noopener">Referrer-Policy</a>') ?></th>
			<td>
		...
		?>
		<tr>
			<th scope="row" class="row-med"><?php esc_html_e('Custom HTTP headers', 'nfwplus'); ?></th>
			<td>
				...
				<p class="description"><?php _e('You can enter one or more custom HTTP headers. The format is <code>header: value</code>, one per line.', 'nfwplus') ?></p>
			</td>
		</tr>

		<tr>
			<th scope="row" class="row-med"><?php esc_html_e('HTTP headers test', 'nfwplus'); ?></th>
			<td>
				<button type="button" class="button-secondary" onClick="nfwjs_test_headers('<?php echo esc_attr( home_url( '/' ) ) ?>');" /><?php esc_html_e('View your website\'s HTTP response headers', 'nfwplus' )?></button>
				&nbsp;&nbsp;&nbsp;
				...
				<p class="description"><?php _e('Make sure to save your changes before running the test.', 'nfwplus') ?></p>
			</td>
		</tr>
	</table>
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('Block PHP built-in wrappers in <code>GET</code>, <code>POST</code>, <code>HTTP_USER_AGENT</code>, <code>HTTP_REFERER</code> and cookies', 'nfwplus') ?></th>
			<td>
		...
		?>
		<tr>
			<th scope="row" class="row-mid"><?php _e('Block serialized PHP objects in the following global variables', 'nfwplus') ?></th>
			<td>
		...
		<tr>
			<th scope="row" class="row-mid"><?php _e('Block attempts to override PHP Superglobals', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[php_superglobals]', $yes, $no, 'small', $php_superglobals ) ?>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-mid"><?php _e('Hide PHP notice and error messages', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[php_errors]', $yes, $no, 'small', $php_errors ) ?>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-mid"><?php _e('Sanitise <code>PHP_SELF</code>', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[php_self]', $yes, $no, 'small', $php_self ) ?>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-mid"><?php _e('Sanitise <code>PATH_TRANSLATED</code>', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_options[php_path_t]', $yes, $no, 'small', $php_path_t ) ?>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-mid"><?php _e('Sanitise <code>PATH_INFO</code>', 'nfwplus') ?></th>
			<td>
	...
	if ( strlen( $_SERVER['DOCUMENT_ROOT'] ) < 5 ) {
		...
		$disabled_msg = '<p class="description">' .
							__('This option is not compatible with your actual configuration.', 'nfwplus') .
							'</p>';
	} else {
		$disabled_msg = '';
	}
	...
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('Block the <code>DOCUMENT_ROOT</code> server variable in HTTP request', 'nfwplus') ?></th>
			<td>
		...
		<tr>
			<th scope="row" class="row-med"><?php _e('Block ASCII character 0x00 (NULL byte)', 'nfwplus') ?></th>
			<td>
				<?php nfw_toggle_switch( 'info', 'nfw_rules[block_null_byte]', $yes, $no, 'small', $block_null_byte ) ?>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php _e('Block ASCII control characters 1 to 8 and 14 to 31', 'nfwplus') ?></th>
			<td>
	...
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<input class="button-secondary" type="submit" name="Default" value="<?php _e('Restore Default Values', 'nfwplus') ?>" onclick="return nfwjs_restore_default();" />
	</form>
</div>
...
function nf_sub_policies_save() {
	...
	// Scan ZIP archives too
	if ( isset( $_POST['nfw_options']['scan_zip']) ) {
		// Uploads must be set to "block dangerous files"
		if ( $nfw_options['uploads'] == 2 ) {
			$nfw_options['scan_zip'] = 1;
		} else {
			$nfw_options['scan_zip'] = 0;
		}
	} else {
		$nfw_options['scan_zip'] = 0;
	}
	...
	// Disable error handler (WordPress 5.1))
