...
function nfw_sys_events() {
	...
	$label_name		= __('Name:', 'nfwplus');
	$label_plugin	= __('Plugin', 'nfwplus');
	$label_theme	= __('Theme', 'nfwplus');
	$label_version	= __('Version:', 'nfwplus');
	$alert_action	= 0;
	...
	if ( strpos( $script, '/themes.php') !== FALSE ) {
		...
		if ( current_user_can('switch_themes') && isset( $_GET['action'] ) ) {
			...
			if ( $_GET['action'] == 'activate' && ! empty( $nfw_options['a_23'] ) ) {
				...
				$alert_action	= sprintf( '%s %s', $label_theme, __('activated', 'nfwplus') );
				...
				$theme			= wp_get_theme( $_GET['stylesheet'] );
				$alert_action	= sprintf( '%s %s', $label_theme, __('deleted', 'nfwplus') );
				$alert_item		= sprintf( '%s %s', $label_name, $theme );
			}
		}
	 ...
	 */
		...
		if ( $_REQUEST['action'] == 'activate' && ! empty( $nfw_options['a_13'] ) ) {
			$alert_action	= sprintf( '%s %s', $label_plugin, __('activated', 'nfwplus') );
			...
			$alert_action	= sprintf( '%s %s', $label_plugin, __('updated', 'nfwplus') );
			$alert_item		= sprintf( '%s %s', $label_name, $plugin_list );

		} elseif ( $_REQUEST['action'] == 'deactivate' && current_user_can( 'deactivate_plugin' ) &&
			! empty( $nfw_options['a_15'] ) ) {

			$alert_action	= sprintf( '%s %s', $label_plugin, __('deactivated', 'nfwplus') );
			$alert_item		= sprintf( '%s %s', $label_name, $plugin );
			...
			$alert_action	= sprintf( '%s %s', $label_plugin, __('deleted', 'nfwplus') );
			$alert_item		= sprintf( '%s %s', $label_name, $plugin_list );
		}
	...
	// update-core.php (only used for WP updates)
		...
		if ( $_GET['action'] == 'do-core-upgrade' && current_user_can( 'update_core' ) &&
			...
			$alert_action	= sprintf( '%s %s', 'WordPress', __('updated', 'nfwplus') );
	...
	// update.php
		...
		if ( $_GET['action'] == 'update-selected' && current_user_can( 'update_plugins' ) &&
			...
			$alert_action	= sprintf( '%s %s', $label_plugin, __('installed', 'nfwplus') );
			$alert_item		= sprintf( '%s %s', $label_name, @$_REQUEST['plugin'] );
			...
			if ( isset( $_FILES['pluginzip']['name'] ) ) {
				$alert_action	= sprintf( '%s %s', $label_plugin, __('uploaded', 'nfwplus') );
			...
			$alert_action	= sprintf( '%s %s', $label_theme, __('updated', 'nfwplus') );
			...
			$alert_action	= sprintf( '%s %s', $label_theme, __('installed', 'nfwplus') );
			$alert_item		= sprintf( '%s %s', $label_name, @$_REQUEST['theme'] );

		} elseif ( $_GET['action'] == 'upload-theme' && current_user_can( 'upload_themes' ) &&
			! empty( $nfw_options['a_21'] ) ) {

			$alert_action	= sprintf( '%s %s', $label_theme, __('uploaded', 'nfwplus') );
