...
if (! empty($_REQUEST['nfw_act'])) {
	...
	if ( $_REQUEST['nfw_act'] == 'create') {
		if (! $err = nf_sub_monitoring_create($nfmon_snapshot) ) {
			$success = __('Snapshot successfully created.', 'nfwplus');
		...
		if (file_exists($nfmon_snapshot) ) {
			...
			$success = __('Snapshot file successfully deleted.', 'nfwplus');
		...
		if (! file_exists($nfmon_snapshot) ) {
			$err = __('You must create a snapshot first.', 'nfwplus');
		} else {
			...
			if (! $err) {
				if (file_exists($nfmon_diff) ) {
					$err =  __('NinjaFirewall detected that changes were made to your files.', 'nfwplus');
					$changes = 1;
				} else {
					$success =  __('No changes detected.', 'nfwplus');
				}
			}
		}
	} elseif ( $_REQUEST['nfw_act'] == 'scheduled') {
		nf_scheduled_scan();
		$success = __('Your changes have been saved.', 'nfwplus');
	}
}
...
if (! file_exists($nfmon_snapshot) ) {
	...
	<div class="nfw-notice nfw-notice-orange">
		<p><?php _e('File Check lets you perform file integrity monitoring upon request or on a specific interval. To start, create a snapshot of your files by clicking the button below.', 'nfwplus' ) ?></p>
	</div>
	...
	<form method="post" name="monitor_form">
		...
		<table class="form-table nfw-table">
			<tr>
				<th scope="row" class="row-med"><?php _e('Create a snapshot of all files stored in that directory', 'nfwplus') ?></th>
				...
				?>" required />
				<p class="description"><?php printf( __('Default: %s', 'nfwplus'), htmlentities( rtrim( ABSPATH, '/\\ ' ) ) ) ?></p>
				</td>
			</tr>

			<tr>
				<th scope="row" class="row-med"><?php _e('Exclude the following files/folders (optional)', 'nfwplus') ?></th>
				<td><input class="large-text" type="text" name="snapexclude" value="<?php echo htmlspecialchars($nfw_options['snapexclude']); ?>" placeholder="<?php _e('e.g.,', 'nfwplus') ?> /wp-content/nfwlog/" maxlength="255"><br /><p class="description"><?php _e('Full or partial case-sensitive string(s). Multiple values must be comma-separated', 'nfwplus') ?> (<code>,</code>).</p></td>
			</tr>

			<tr>
				<th scope="row" class="row-med">&nbsp;</th>
				<td><label><input type="checkbox" name="snapnoslink" value="1" checked="checked"/><?php _e('Do not follow symbolic links (default)', 'nfwplus') ?></label></td>
			</tr>

		</table>
		<input type="hidden" name="nfw_act" value="create" />
		<p><input type="submit" class="button-primary" value="<?php _e('Create Snapshot', 'nfwplus') ?>" /></p>
...
if (file_exists($nfmon_diff) ) {
 ...
 ?>
	<table class="form-table nfw-table">
		<tr>
			<th scope="row" class="row-med"><?php _e('Last snapshot', 'nfwplus') ?></th>
			<td>
				<p><?php printf( __('Created on: %s', 'nfwplus'), date_i18n('M d, Y @ H:i:s O', $stat['ctime'])); ?></p>
				<p><?php printf( __('Total files: %s ', 'nfwplus'), number_format_i18n($count) ); ?></p>

				<p><?php _e('Directory:', 'nfwplus') ?><code><?php echo htmlspecialchars($nfw_options['snapdir']) ?></code></p>
				<?php
				if (! empty($nfw_options['snapexclude']) ) {
					$res = @explode(',', $nfw_options['snapexclude']);
					echo '<p>' .  __('Exclusion:', 'nfwplus') . ' ';
					foreach ($res as $exc) {
						echo '<code>' . htmlspecialchars($exc) . '</code>&nbsp;';
					}
					echo '</p>';
				}
				echo	'<p>' .  __('Symlinks:', 'nfwplus') . ' ';
				if ( empty($nfw_options['snapnoslink']) ) {
					echo __('follow', 'nfwplus');
				} else {
					echo __('do not follow', 'nfwplus');
				}
				echo '</p>';
				if (! empty($nfw_options['snapproc']) ) {
					echo '<p>' . sprintf( __('Processing time: %s seconds', 'nfwplus'), $nfw_options['snapproc']) . '</p>';
				}
				?>
				<form method="post">
					<?php wp_nonce_field('filecheck_save', 'nfwnonce', 0); ?>
					<p><input type="submit" name="dlsnap" value="<?php _e('Download Snapshot', 'nfwplus') ?>" class="button-secondary" />&nbsp;&nbsp;&nbsp;<input type="submit" class="button-secondary" onClick="return nfwjs_del_snapshot();" value="<?php _e('Delete Snapshot', 'nfwplus') ?>" /><input type="hidden" name="nfw_act" value="delete" /></p>
					<input type="hidden" name="tab" value="filecheck" />
				</form>
			</td>
		</tr>
		<tr>
			<th scope="row" class="row-med"><?php _e('Last changes', 'nfwplus') ?></th>
			<td>
			...
			if ($mod) {
			?>
				<p><?php printf( __('New files: %s', 'nfwplus'), count($new_file) ) ?></p>
				<p><?php printf( __('Deleted files: %s', 'nfwplus'), count($del_file) ) ?></p>
				<p><?php printf( __('Modified files: %s', 'nfwplus'), count($mod_file) ) ?></p>

				<form method="post">
					<?php wp_nonce_field('filecheck_save', 'nfwnonce', 0); ?>
					<p><input type="button" value="<?php _e('View Changes', 'nfwplus') ?>" onClick="nfwjs_show_changes();" class="button-secondary" id="vcbtn" <?php
					if (! empty($changes)) {
						echo 'disabled="disabled" ';
					}
					?>/>&nbsp;&nbsp;&nbsp;<input type="submit" name="dlmods" value="<?php _e('Download Changes', 'nfwplus') ?>" class="button-secondary" /></p>
				</form>
				<br />
			<?php
				if (empty($changes)) {
				...
				$more_info = __('Click a file to get more info about it.', 'nfwplus');
				if ($new_file) {
					echo '<tr><td>';
					echo __('New files:', 'nfwplus') . ' ' . count($new_file). '<br />';
					...
					<table id="table_new" style="width:100%;background-color:#F7F7F7;border:solid 1px #DFDFDF;display:none;">
						<tr>
							<th style="padding:2px;width:25%;">' . __('Size', 'nfwplus') .'</th>
							<td style="padding:2px" id="new_size"></td>
						</tr>
						<tr>
							<th style="padding:2px;width:25%;">' . __('Access', 'nfwplus') .'</th>
							<td style="padding:2px" id="new_chmod"></td>
						</tr>
						<tr>
							<th style="padding:2px;width:25%;">' . __('Uid / Gid', 'nfwplus') .'</th>
							<td style="padding:2px" id="new_uidgid"></td>
						</tr>
						<tr>
							<th style="padding:2px;width:25%;">' . __('Modify', 'nfwplus') .'</th>
							<td style="padding:2px" id="new_mtime"></td>
						</tr>
						<tr>
							<th style="padding:2px;width:25%;">' . __('Change', 'nfwplus') .'</th>
				...
				if ($del_file) {
					echo '
			<tr>
				<td>' . __('Deleted files:', 'nfwplus') . ' ' . count($del_file). '<br />' .
				...
				if ($mod_file) {
					echo '
			<tr>
				<td>' . __('Modified files:', 'nfwplus') . ' ' . count($mod_file). '<br />' .
					...
					<table id="table_mod" style="width:100%;background-color:#F7F7F7;border:solid 1px #DFDFDF;display:none;">
						<tr>
							...
							<td style="padding:2px"><b>' . __('Old', 'nfwplus') .'</b></td>
							<td style="padding:2px"><b>' . __('New', 'nfwplus') .'</b></td>
						</tr>
						<tr>
							<th style="padding:2px;width:25%;">' . __('Size', 'nfwplus') .'</th>
	...
	<form method="post">
		...
		<p><input type="submit" class="button-primary" value="<?php _e('Scan System For File Changes', 'nfwplus') ?> &#187;" /></p>
		<input type="hidden" name="tab" value="filecheck" />
	</form>
	...
	<form method="post">
		...
		?>
			<p class="description" style="color:red"><?php printf( __('It seems that %s is set. Ensure you have another way to run WP-Cron, otherwise NinjaFirewall scheduled scans will not work.', 'nfwplus'), '<code>DISABLE_WP_CRON</code>' ) ?></p>
		<?php
		}
		?>
		<table class="form-table nfw-table">
			<tr>
				<th scope="row" class="row-med"><?php _e('Enable scheduled scans', 'nfwplus') ?></th>
				<td>
					<p><label><input type="radio" name="sched_scan" value="0"<?php checked($sched_scan, 0) ?> /><?php _e('No (default)', 'nfwplus') ?></label></p>
					<p><label><input type="radio" name="sched_scan" value="1"<?php checked($sched_scan, 1) ?> /><?php _e('Hourly', 'nfwplus') ?></label></p>
					<p><label><input type="radio" name="sched_scan" value="2"<?php checked($sched_scan, 2) ?> /><?php _e('Twicedaily', 'nfwplus') ?></label></p>
					<p><label><input type="radio" name="sched_scan" value="3"<?php checked($sched_scan, 3) ?> /><?php _e('Daily', 'nfwplus') ?></label></p>
					<?php
					if ( $nextscan = wp_next_scheduled('nfscanevent') ) {
						...
						$diff = $now->diff($sched);
					?>
						<p class="description"><?php printf( __('Next scan will start in approximately %s day(s), %s hour(s), %s minute(s) and %s second(s).', 'nfwplus'), $diff->format('%a') % 7, $diff->format('%h'), $diff->format('%i'), $diff->format('%s') ) ?></p>
					<?php
						...
						if ( $nextscan < time() ) {
						?>
							<p class="description" style="color:red"><?php _e('The next scheduled scan date is in the past! WordPress wp-cron may not be working or may have been disabled.', 'nfwplus'); ?></p>
						<?php
						}
					}
					?>
			...
			<tr>
				<th scope="row" class="row-med"><?php _e('Scheduled scan report', 'nfwplus') ?></th>
				<td>
					<p><label><input type="radio" name="report_scan" value="0"<?php checked($report_scan, 0) ?> /><?php _e('Send me a report by email only if changes are detected (default)', 'nfwplus') ?></label></p>
					<p><label><input type="radio" name="report_scan" value="1"<?php checked($report_scan, 1) ?> /><?php _e('Always send me a report by email after a scheduled scan', 'nfwplus') ?></label></p>
				</td>
			</tr>
		</table>
		<input type="hidden" name="nfw_act" value="scheduled" />
		<p><input type="submit" class="button-primary" value="<?php _e('Save Scan Options', 'nfwplus') ?>" /></p>
		<input type="hidden" name="tab" value="filecheck" />
	</form>
<?php
...
function nf_sub_monitoring_create($nfmon_snapshot) {
	...
	if ( empty($_POST['snapdir']) ) {
		return __('Enter the full path to the directory to be scanned.', 'nfwplus');
	}
	if ( strpos( $_POST['snapdir'], '://' ) !== false ) {
		return sprintf( __('The directory %s is not correct.', 'nfwplus'), '<code>'. htmlspecialchars($_POST['snapdir']) .'</code>');
	}
	...
	if (! file_exists($_POST['snapdir']) ) {
		return sprintf( __('The directory %s does not exist.', 'nfwplus'), '<code>'. htmlspecialchars($_POST['snapdir']) .'</code>');
	}
	if (! is_readable($_POST['snapdir']) ) {
		return sprintf( __('The directory %s is not readable.', 'nfwplus'), '<code>'. htmlspecialchars($_POST['snapdir']) .'</code>');
	}
	...
	if ($fh = fopen($nfmon_snapshot, 'w') ) {
		...
		return sprintf( __('Cannot write to %s.', 'nfwplus'), '<code>'. $nfmon_snapshot .'</code>');
	}
}
...
function scd($snapdir, $snapexclude, $fh, $snapnoslink) {
	...
	if (is_readable($snapdir) ) {
		if ($dh = opendir($snapdir) ) {
			...
			return sprintf(__('Error : cannot open %s directory.', 'nfwplus'), '<code>'. htmlspecialchars($snapdir) .'</code>');
		}
	} else {
		return sprintf(__('Error : %s directory is not readable.', 'nfwplus'), '<code>'. htmlspecialchars($snapdir) .'</code>');
	}
}
...
function nf_sub_monitoring_scan($nfmon_snapshot, $nfmon_diff) {
	...
	if (! isset($nfw_options['snapexclude']) || ! isset($nfw_options['snapdir']) || ! isset($nfw_options['snapnoslink']) ) {
		return sprintf( __('Missing options line %s, please try again.', 'nfwplus'), __LINE__ );
	}
	...
	if ($fh = fopen($nfmon_snapshot . '_tmp', 'w') ) {
		...
		return sprintf( __('Cannot create %s.', 'nfwplus'), '<code>'. $nfmon_snapshot . '_tmp</code>');
	}
	...
	if (! $fh = fopen($nfmon_snapshot, 'r') ) {
		return __('Error reading old snapshot file.', 'nfwplus');
	}
	while (! feof($fh) ) {
	...
	if (! $fh = fopen($nfmon_snapshot . '_tmp', 'r') ) {
		return __('Error reading new snapshot file.', 'nfwplus');
	}
	while (! feof($fh) ) {
		$match = explode('::', rtrim(fgets($fh)) . '::' );
