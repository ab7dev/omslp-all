...
function nfw_fullwaf_htaccess( $httpserver ) {
	...
	if ( file_exists( $NFW_ABSPATH .'.htaccess' ) ) {
		if (! is_writable( $NFW_ABSPATH .'.htaccess' ) ) {
			return sprintf(
				__('Error: Your .htaccess file is not writable, please change its permissions: %s', 'nfwplus' ),
	...
	if ( $httpserver == 1 ) { // Apache mod_php
		...
		return sprintf( __('Error: wrong parameter value (%s).', 'nfwplus'), 'HTTP server' );
	}
	...
	if ( $res === false ) {
		return sprintf(
			__('Error: The following file is not writable, please change its permissions: %s', 'nfwplus' ),
			htmlspecialchars( $NFW_ABSPATH .'.htaccess' )
		);
	}
...
function nfw_waf_sandbox() {
	...
	$sandbox_error = __('NinjaFirewall detected that the requested changes seemed to crash your blog. %s', 'nfwplus') ."\n".
		__('Changes have been undone. You may need to modify your selection and try again.', 'nfwplus' );
	...
	if (! is_wp_error( $res ) ) {
		...
		if ( $res['response']['code'] >= 400 ) {
			$error_msg = sprintf(
				$sandbox_error,
				sprintf(
					__('The website front-end returned: HTTP %s %s.', 'nfwplus'),
		...
		if ( stripos( $res['body'], $blogname ) === false ) {
			$error_msg = sprintf(
				$sandbox_error,
				__('The website front-end did not return the expected page.', 'nfwplus')
			);
			return $error_msg;
		}
		...
		$error_msg = sprintf(
			$sandbox_error,
			sprintf(
				__('The website front-end returned a fatal error: %s.', 'nfwplus'),
	...
	if ( $wp_config == true ) {
		...
		if ( file_exists( $wp_config ) ) {
			if ( is_writable( $wp_config ) ) {
				...
				$res['wp-config'] = __('File is not writable', 'nfwplus');
			}
		}
	}
	...
	if ( $htaccess == true ) {
		...
		if ( file_exists( $htaccess ) ) {
			if ( is_writable( $htaccess ) ) {
				...
				$res['htaccess'] = __('File is not writable', 'nfwplus');
			}
		}
	}
	...
	if ( $ini == true ) {
		...
		if ( file_exists( $ini ) ) {
			if ( is_writable( $ini ) ) {
				...
				$res['ini'] = __('File is not writable', 'nfwplus');
			}
		}
