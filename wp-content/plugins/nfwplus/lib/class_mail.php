...
class NinjaFirewall_mail {
	...
	private static function initialize( $what, $dir = '') {
		 ...
		 */
		if ( $what == 'firewall') {
			$file = '/mail_template_firewall_pro.php';
			$logdir = $dir;
		} else {
			$file = '/mail_template_plugin_pro.php';
		...
		foreach( $recipients as $to ) {
				...
				*/
				if ( $to != $admin_email ) {
					...
					$click	= "\n\n". sprintf(
						...
						__("If you don't have access to that site any longer, you can remove your email by clicking the following link (valid for 12 hours): %s", 'nfwplus'),
						$uri
					);
				}
			}
			...
			if ( $res === false ) {
				nfw_log_error( sprintf(
					...
					__('Cannot send email "%1$s" to recipient "%2$s"', 'nfwplus'),
					$subject, $to
				) );
			}
			/**
