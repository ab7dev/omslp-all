...
function nfw_check_security_updates() {
	...
	if (! isset( $list['wordpress'] ) || ! isset( $list['themes'] ) || ! isset( $list['plugins'] ) ) {
		nfw_log_error(	__('Downloaded list of vulnerabilities is corrupted', 'nfwplus' ) );
		return false;
	}
...
function nfw_alert_security_updates( $found = [] ) {
	...
	if (! empty( $found['wordpress'] ) ) {
		$message .= "WordPress:\n" .
			sprintf( __('Your version: %s', 'nfwplus'), $found['wordpress']['cur_version'] ) ."\n".
			sprintf( __('New version: %s', 'nfwplus'), $found['wordpress']['new_version'] ) ."\n";
		if ( $found['wordpress']['level'] == 2 ) {
			$message .= __('Severity: This is an important security update', 'nfwplus') ."\n";
		} elseif ( $found['wordpress']['level'] == 3 ) {
			$message .= __('Severity: **This is a critical security update**', 'nfwplus') ."\n";
		}
		$message .= "\n";
	}
	...
	if (! empty( $found['plugins'] ) ) {
		foreach( $found['plugins'] as $k => $v ) {
			$message .= sprintf( __('Plugin: %s', 'nfwplus'), $found['plugins'][$k]['name'] ) ."\n".
				sprintf( __('Your version: %s', 'nfwplus'), $found['plugins'][$k]['cur_version'] ) ."\n".
				sprintf( __('New version: %s', 'nfwplus'), $found['plugins'][$k]['new_version'] ) ."\n";
	...
	if (! empty( $found['themes'] ) ) {
		...
		foreach( $found['themes'] as $k => $v ) {
			$message .= sprintf( __('Theme: %s', 'nfwplus'), $found['themes'][$k]['name'] ) ."\n".
				sprintf( __('Your version: %s', 'nfwplus'), $found['themes'][$k]['cur_version'] ) ."\n".
				sprintf( __('New version: %s', 'nfwplus'), $found['themes'][$k]['new_version'] ) ."\n";
...
function nfw_fetch_security_updates( $url ) {
	...
	if ( is_wp_error( $res ) ) {
		nfw_log_error( __('Cannot download security rules: connection error. Will try again later', 'nfwplus') );
		return false;
	}

	if ( $res['response']['code'] != 200 ) {
		nfw_log_error(
			sprintf( __('Cannot download security rules: HTTP response error %s. Will try again later', 'nfwplus'), $res['response']['code'] )
		);
		return false;
	}
