...
class MailsterFrontpage {
	...
	private function do_tracking_actions() {
		...
		if ( $target ) {
			...
			// check if it's a status where we actually need to check the content
			$valid_status = in_array( $campaign->post_status, array( 'active', 'paused', 'finished', 'queued', 'workflow' ) );

			if ( ! $home_in_target_host && ! $target_in_home_host && $valid_status ) {
