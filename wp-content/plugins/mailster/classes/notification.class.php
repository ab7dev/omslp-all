...
class MailsterNotification {
	...
	public function filter( $content, $template, $subscriber, $options ) {
		...
		switch ( $template . '_' . $filter ) {
			...
			case 'confirmation_subject':
				$form = $this->get_form_from_subscriber( $subscriber, $options );

				return $form->subject ? $form->subject : sprintf( esc_html__( 'Welcome to %s! Please Confirm Your Email', 'mailster' ), '{company}' );

			case 'confirmation_file':
				$form = $this->get_form_from_subscriber( $subscriber, $options );

				return $form->template ? $form->template : 'notification.html';

			case 'confirmation_headline':
				$form = $this->get_form_from_subscriber( $subscriber, $options );

				return $form->headline ? $form->headline : esc_html__( 'Please confirm your Email', 'mailster' );

			case 'confirmation_replace':
				$form = $this->get_form_from_subscriber( $subscriber, $options );

				$link_text = $form->link ? $form->link : esc_html__( 'Confirm your email address', 'mailster' );
	...
	private function get_form_from_subscriber( $subscriber, $options ) {
		$form_id = mailster( 'subscribers' )->meta( $subscriber->ID, 'form' );
		$form    = get_post( $form_id );

		// no form found => get legacy form first, then default form
		if ( ! $form || $form->post_type != 'mailster-form' ) {

			// legacy form is set in options
			$leagcy_form = $this->get_form_options( $options['form'], $subscriber );
			if ( $leagcy_form ) {
				return $leagcy_form;
			}
	...
	private function parse_form( $form ) {
		...
		// convert ot the needed format
		$myForm = array(
			'ID'              => $form->ID,
			'name'            => $form->post_title,
			//'submit' => Subscribe,
			//'asterisk' => 1,
			'userschoice'     => get_post_meta( $form->ID, 'userschoice', true ),
			//'precheck'        => get_post_meta( $form->ID, 'precheck', true ),
			'dropdown'        => get_post_meta( $form->ID, 'dropdown', true ),
			'prefill'         => get_post_meta( $form->ID, 'prefill', true ),
			//'inline' => get_post_meta( $form->ID, 'inline', true ),
			'overwrite'       => get_post_meta( $form->ID, 'overwrite', true ),
			//'addlists' => 0,
			//'style' => get_post_meta( $form->ID, 'style', true ),
			'doubleoptin'     => get_post_meta( $form->ID, 'doubleoptin', true ),
			'subject'         => get_post_meta( $form->ID, 'subject', true ),
			'headline'        => get_post_meta( $form->ID, 'headline', true ),
			'content'         => get_post_meta( $form->ID, 'content', true ),

			'link'            => get_post_meta( $form->ID, 'link', true ),
			//'resend' => 0,
			//'resend_count' => 2,
			//'resend_time' => 48,
			'template'        => 'notification.html',
			'vcard'           => false,
			'confirmredirect' => get_post_meta( $form->ID, 'confirmredirect', true ),
			'redirect'        => get_post_meta( $form->ID, 'redirect', true ),

			'tags'            => array(),

			'gdpr'            => get_post_meta( $form->ID, 'gdpr', true ),

		);

		return (object) $myForm;
	}
	...
	private function get_default_form() {
		...
		return $this->parse_form( $forms[0] );
	}
	...
	private function template_confirmation( $subscriber, $options ) {
		...
		<div itemscope itemtype="http://schema.org/EmailMessage">
			...
			<meta itemprop="name" content="<?php esc_attr_e( 'Confirm Subscription', 'mailster' ); ?>"/>
			<div itemprop="handler" itemscope itemtype="http://schema.org/HttpActionHandler">
			<link itemprop="url" href="{linkaddress}"/>   
			</div>
			</div>
			<meta itemprop="description" content="<?php esc_attr_e( 'Confirmation Subscription', 'mailster' ); ?>"/>
		</div> 
		<?php
	}
